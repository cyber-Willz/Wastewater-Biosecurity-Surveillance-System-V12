...
## Appendix D — v0.6 changes: backtesting rigor, site weighting, spectral gate, audit chain

### D.1 What changed

| # | Item | Change in v0.6 |
|---|---|---|
| 1 | Backtesting granularity | `ww_eval/src/bin/real_data.rs`: per-wave hit/miss replaced with a `(wave, severity_floor)` sensitivity/specificity/PPV confusion matrix at `(site, week)` granularity |
| 2 | Site reliability | New `SiteWeight`/`SiteWeightRegistry`; per-site `z_eff = z_threshold/√w` and a minimum-sampling-density gate on alert generation (not baseline updates) |
| 3 | Spectral term validity | New `SpectralMode::{Enabled, Gated, Disabled}`; default `Gated{min_real_catchments: 2}` zeroes the severity contribution of the spectral score until real catchment topology is registered |
| 4 | Audit tamper-evidence | `AuditEntry` gains `prev_hash`/`entry_hash` (SHA-256 hash chain); DB trigger rejects chain forks on INSERT; `verify_chain` walks the full chain |

### D.2 Site-weight derivation

The z-score computed by `EwmaBaseline` at a well-instrumented site estimates
the population mean with standard error $\sigma/\sqrt{n}$ for $n$ pooled
samples. A site with fewer effective samples per round, older equipment, or
higher background heterogeneity has an inflated *effective* variance relative
to a well-calibrated reference site. Model this inflation as a multiplicative
factor: the site's true measurement variance is $\sigma^2/w$ for some
reliability weight $w \in (0,1]$ (so $w=1$ is the reference site, $w<1$ is
noisier).

A z-score computed against the *nominal* EWMA variance $\sigma^2$ at a site
whose true variance is $\sigma^2/w$ is therefore over-dispersed by a factor
of $1/\sqrt w$ relative to what a correctly-calibrated z-score would show — the
same z-value corresponds to a less extreme quantile than it would at the
reference site. To hold the **false-alarm rate constant across sites** (the
actual operational goal — an operator should not see Fort William alert twice
as often as a well-calibrated site purely because of higher intrinsic noise,
for the same real quantile of departure from baseline), the alerting
threshold must be raised by the same factor:

$$
z_{\text{eff}} = \frac{z_{\text{threshold}}}{\sqrt{w}}.
$$

At $w=1$ this is the identity (no change, matching every pre-v0.6
deployment). At $w=0.25$ (a site modeled as 4× noisier than reference) the
effective threshold is $2\times$ the nominal one.

**Caveat, stated plainly:** $w$ is not estimated from the data by this
release — it is an operator-supplied configuration value (`PUT
/v1/network/site-weights`). A principled next step is to *estimate* $w$ from
each site's residual variance under the null (e.g. the ratio of the site's
empirical Welford variance to the fleet median), which would make the weight
self-calibrating; v0.6 ships the mechanism and the manual configuration path,
not the estimator. Until an estimator exists, weights should be set from
known instrumentation quality (flow-proportional vs grab sampling, composite
window length, calibration record), not fitted post hoc to suppress a
specific site's alert rate — doing the latter defeats the purpose of the
gate.

The minimum-sampling-density gate (`min_samples`) is a separate, simpler
mechanism: below a configured sample count the round's statistical power is
judged too low for the EWMA/Welford estimators to be trusted regardless of
$w$, so no alert is raised (the observation still updates the baseline, so
history is not lost and the site is not permanently silenced).

### D.3 Spectral-gate rationale

§4.7–4.8 (above) already established that the spectral term's *statistic* is
well-defined regardless of network topology — the kernel-mismatch and
null-model sections hold for any hypergraph, real or not. What D.3 adds is an
operational point, not a mathematical correction: a well-defined statistic
computed on a network built from health-board administrative boundaries (the
`real_data.rs` proxy, used because the public PHS dataset does not publish
sewer connectivity) measures roughness with respect to *that* grouping, which
has no claim to reflecting actual hydraulic coupling between sites. Using its
output to upgrade severity therefore risks encoding administrative geography
as if it were physical correlation.

`SpectralMode::Gated` does not change the statistic; it changes whether the
statistic is allowed to move an alert's severity tier. The raw score is
always computed and stored on every `AnomalyEvent`, so nothing is lost for
retrospective analysis — only the automatic severity upgrade is withheld
until `n_real_catchments` (catchments with ≥ 2 real, sewer-connected member
sites) clears the configured floor.

### D.4 Audit chain: what "hash-chained" does and does not guarantee here

Standard hash-chain construction (as in e.g. a simplified blockchain or a
transparency log): each entry commits to the previous entry's hash, so any
retroactive edit to entry $i$ requires recomputing every hash from $i$ onward
to keep the chain internally consistent. This is **tamper-evidence**, not
**tamper-proofness** — a party with write access to the full table (e.g. a
database administrator, or an attacker with equivalent access) can rewrite
the entire sequence and produce a chain that verifies successfully end to
end. What the chain rules out is *partial* tampering — editing one entry
without touching the rest — which is the realistic threat model for most
insider-misuse and error-correction scenarios, but not for a fully compromised
database host.

Closing the remaining gap (detecting a full-sequence rewrite) requires an
external, independent commitment to the chain tip at each point in time:
periodically signing `AuditLog::chain_tip()` with a hardware security module,
or submitting it to a third-party RFC 3161 timestamping authority or a public
append-only log. Neither is implemented in v0.6; both are deployment
decisions that depend on the operator's threat model and are out of scope for
this codebase.
